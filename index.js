...
// session 中间件
app.use(session({
  name: config1.session.key, // 设置 cookie 中保存 session id 的字段名称
  secret: config1.session.secret, // 通过设置 secret 来计算 hash 值并放在 cookie 中，使产生的 signedCookie 防篡改
  resave: true, // 强制更新 session
  saveUninitialized: false, // 设置为 false，强制创建一个 session，即使用户未登录
  cookie: {
    maxAge: config1.session.maxAge// 过期时间，过期后 cookie 中的 session id 自动删除
  }
...
// 监听端口，启动程序
app.listen(config1.post, function () {
